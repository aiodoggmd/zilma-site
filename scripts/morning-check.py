...
случай»):
  ...
  Заявки       форма отправляет туда, где её ждут, и приёмник отвечает. Адрес берётся из
               собранного скрипта формы, а не из наших предположений — см. form_endpoint().
               Запрос уходит с заполненной ловушкой для ботов (hp_field),
...
def form_endpoint() -> str:
    """Адрес, на который форма РЕАЛЬНО отправляет заявку — вытащен из собранного скрипта
    формы на живом сайте.

    Раньше проверка дёргала order.php напрямую и отчитывалась «приёмник отвечает». 11.09.2026
    это пропустило настоящую аварию: приёмник работал, а форма после моей выкладки стреляла
    в '/api/order' — маршрут Vercel, которого на статическом сайте нет. Клиенты полдня
    видели «не получилось отправить», и узнали мы об этом от клиентки, а не от проверки.
    Поэтому адрес берётся из того же файла, который исполняется в браузере покупателя.
    """
    status, _, body = fetch(f'{SITE}/order/')
    if status != 200:
        raise Problem(f'страница заявки отдаёт {status} — заказать с сайта нельзя')
    m = re.search(r'/_astro/(OrderForm[\w.-]+\.js)', body.decode('utf-8', 'replace'))
    if not m:
        raise Problem('на странице заявки не нашёлся скрипт формы — проверь сборку')
    status, _, js = fetch(f'{SITE}/_astro/{m.group(1)}')
    if status != 200:
        raise Problem(f'скрипт формы отдаёт {status} — форма на сайте не работает')
    # Сборщик оставляет адрес шаблонной строкой в обратных кавычках (`fetch(`/order.php`,…)`),
    # а не в обычных — на этом первая версия проверки и споткнулась. Принимаем любые кавычки.
    m = re.search(r'fetch\(([`"\'])(/[^`"\']+)\1', js.decode('utf-8', 'replace'))
    if not m:
        raise Problem('в скрипте формы не нашёлся адрес отправки — проверь OrderForm.astro')
    return m.group(2)


def check_orders() -> str:
    """Побочных действий нет: заполненная ловушка для ботов заставляет приёмник ответить
    и выйти до сохранения заявки, писем и счётчика частоты."""
    endpoint = form_endpoint()
    data = urllib.parse.urlencode({'hp_field': 'morning-check'}).encode()
    status, _, body = fetch(SITE + endpoint, data=data,
                            headers={'Content-Type': 'application/x-www-form-urlencoded'})
    if status == 404:
        raise Problem(f'форма отправляет заявки на {endpoint}, а такого адреса на сайте нет — '
                      f'заказы НЕ доходят; проверь PUBLIC_ORDER_ENDPOINT и пересобери сайт')
    if status == 500:
    ...
    try:
        ...
        ok = False
    if not ok:
        raise Problem(f'приёмник {endpoint} ответил не так, как должен — проверь order.php')
    return f'форма шлёт на {endpoint}, приёмник отвечает'
