...
def main() -> int:
    ...
    try:
        ...
        # Приём заявок — самое ценное на сайте, поэтому прежний файл сохраняется как .bak
        # ДО замены: если новый окажется битым, откат — одно переименование на хостинге.
        #
        # НО копия живёт только до ближайшего `npm run publish`: тот сверяет сервер со
        # свежей сборкой и сносит всё лишнее, а .bak в списке KEEP не значится (проверено
        # 21.09.2026 — снёс через полчаса). Поэтому .bak — подстраховка на ближайшие
        # минуты, а НАСТОЯЩИЙ откат такой:
        #     git checkout <прежний коммит> -- deploy/
        #     .venv/Scripts/python.exe scripts/deploy-php.py --apply
        # Он не зависит от того, что осталось на сервере.
        print()
        for name, data, digest, size in local:
            ...
            if not ok:
                print("     ОТКАТ: git checkout <прежний коммит> -- deploy/ и запустить этот скрипт снова")
