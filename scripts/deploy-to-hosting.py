...
#
# mailer.php тоже сюда: его подключает order.php, и без него заявка сохраняется, а письмо
# не уходит. В первый список он не попал, и ближайшая же выкладка собиралась его снести
# (поймано на предпросмотре 2026-09-11). Правило: любой файл, который лежит на хостинге
# и не создаётся сборкой, добавляется СЮДА в тот же заход, когда появляется на сервере.
KEEP = {'.htaccess', 'order.php', 'orders.php', 'config.php', 'mailer.php'}
