...
def bubble(size: int = 140) -> Image.Image:
    """Стеклянный пузырь вместо плоского белого круга — для осветляющих LT и LT-EX.

    Плоская белая заливка рядом с фото настоящих прядей выглядела дырой в палитре
    (замечание пользователя 11.09.2026). Пузырь решает обе задачи сразу: цвета у него
    по-прежнему нет, но он объёмный и читается как предмет, а не как пропуск.

    Свет считается по нормали сферы, а не рисуется вручную градиентом: у настоящей сферы
    блик смещён к источнику света, а край темнеет — глазу это и даёт объём. Прозрачность
    падает к центру: сквозь середину видно страницу, поэтому кружок остаётся «без цвета».
    Рисуем с четырёхкратным запасом и уменьшаем — иначе край получается ступенчатым.
    """
    ss = size * 4
    y, x = np.mgrid[0:ss, 0:ss]
    # Координаты от -1 до 1 с центром в середине круга.
    nx = (x - ss / 2) / (ss / 2)
    ny = (y - ss / 2) / (ss / 2)
    r2 = nx ** 2 + ny ** 2
    inside = r2 <= 1.0
    ...
    # Источник света сверху слева и чуть спереди.
    lx, ly, lz = -0.45, -0.6, 0.66
    diffuse = np.clip(nx * lx + ny * ly + nz * lz, 0, 1)

    # Тело почти белое: от лёгкой серизны у нижнего края до чистого белого под светом.
    value = 232 + diffuse * 23
    # Узкий блик — та самая «капля» на стекле.
    spec = np.exp(-((nx - lx * 0.55) ** 2 + (ny - ly * 0.55) ** 2) / 0.045) * 60
    value = np.clip(value + spec, 0, 255)

    # Прозрачность: середина почти прозрачная, к краю стекло «набирает толщину».
    alpha = 0.30 + 0.70 * np.clip(r2, 0, 1) ** 1.6
    # Тонкий плотный ободок по самому краю, иначе пузырь растворяется на белом фоне.
    alpha = np.where(r2 > 0.90, 0.96, alpha)
    alpha = np.clip(alpha * inside, 0, 1)

    rgb = np.dstack([value] * 3).astype(np.uint8)
...
def main() -> None:
    ...
    # Пузырь рисуем сами, а не берём с карты: см. комментарий к LIFTERS выше. Он идёт
    # картинкой, а не цветом фона, ещё и ради формы — свотчи с картинкой круглые, а без
    # неё квадратные, и два квадрата среди сотни кругов выглядели бы как сбой.
    # lossless: прозрачность и мягкий блик webp с потерями заметно грязнит.
    for idx, (code, _what) in enumerate(LIFTERS):
        slug = code.lower()
        bubble().save(IMG_DIR / f'{slug}.webp', 'WEBP', quality=95, method=6, lossless=True)
