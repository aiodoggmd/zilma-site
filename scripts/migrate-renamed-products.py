...
JOURNALS = [
    ...
    # Тоже словарь по имени: без переноса переименованный товар выпадет из своего
    # раздела обратно в плоский список под брендом.
    ROOT / 'src' / 'data' / 'price-sections.json',
    # Промо-сайдкар: старая цена и процент скидки по КАЖДОМУ акционному товару.
    # Тоже словарь по имени, и без переноса переименованный товар теряет
    # перечёркнутую старую цену и процент — акция превращается в обычную цену.
    # Поймано 13.09.2026 при переносе имён из каталога: 140 из 296 акционных
    # остались без старой цены, и заметно это стало только по строке
    # «N акционных товаров не нашлись в сайдкаре» в xlsx-to-price-items.py.
    ROOT / 'Price' / 'promo-meta.json',
]
