...
/* --- Куда уходит заявка ------------------------------------------------------------
   Решение пользователя 2026-09-10, уточнено 12.09.2026: заявки идут ТОЛЬКО НА ПОЧТУ.
   MAX задумывался вторым каналом, но настраивать его не стали — отдельная возня, а
   почты достаточно. Сервер почты в России, значит персональные данные страну не
   покидают, трансграничной передачи нет.

   MAX и Telegram ниже включаются непустыми ключами в config.php и сейчас оба молчат.
   Telegram с российского хостинга недоступен в принципе: висел 25 секунд, браузер
   показывал «Failed to fetch». Он и получал бы только состав и сумму — серверы за
   границей.

   ВНИМАНИЕ, ПРЕЖДЕ ЧЕМ ВКЛЮЧАТЬ ЛЮБОЙ ИЗ НИХ: у обоих timeout 20 секунд, а
   max_execution_time на хостинге — 10. Ответ клиенту держится до отправки (см. абзац
   выше), поэтому медленный канал повторит ту же аварию. Включать — только вынеся
   отправку в отдельную задачу после ответа клиенту.
