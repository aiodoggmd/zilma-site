...
// Буфер включаем сразу: он нужен, чтобы корректно закрыть соединение с браузером
// до отправки уведомлений (см. блок «Сначала ответ клиенту» ниже).
ob_start();
...
/* --- Сначала ответ клиенту, потом уведомления -------------------------------------
   Реальный баг, пойманный пользователем 10.09.2026: заявка сохранялась, но человек видел
   «Failed to fetch». Причина — скрипт держал соединение, пока ломился в Telegram, а тот с
   российского хостинга недоступен и отдаёт таймаут 25 секунд. Браузер столько не ждёт.

   Поэтому: заявка уже на диске, отвечаем «принято» и закрываем соединение, а письма и
   сообщения шлём после — их задержка клиента больше не касается. */
echo json_encode(['ok' => true, 'id' => $id], JSON_UNESCAPED_UNICODE);
if (function_exists('fastcgi_finish_request')) {
    fastcgi_finish_request();
} else {
    // Запасной путь, если PHP работает не через FPM: браузер получает ответ и уходит,
    // а скрипт доживает своё в фоне.
...
// Ответ клиенту отправлен выше, до уведомлений — см. пояснение там же.
