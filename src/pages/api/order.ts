...
export const POST: APIRoute = async ({ request }) => {
  ...
  //
  // ВАЖНО: метку ставит браузер КЛИЕНТА, а сравнивается она с часами СЕРВЕРА — и они
  // всегда чуть разные. Если часы клиента спешат, возраст формы получается отрицательным
  // («отправлена раньше, чем открыта»), и заявка молча выбрасывалась: человек видел
  // «Заявка отправлена, спасибо!», а заказ не приходил никому. Поймано вживую 2026-09-10
  // на переносе сайта — часы того сервера отставали на 16 секунд, и настоящая заявка
  // исчезла именно так.
  //
  // Поэтому подозрительной считается ТОЛЬКО реально быстрая отправка: возраст формы
  // неотрицательный и меньше порога. Отрицательный возраст — это расхождение часов, а не
  // робот. Пропустить лишнего робота не страшно (его ловит honeypot выше), потерять заказ
  // живого клиента — страшно.
  const formAgeMs = Date.now() - renderedAt;
  const timingSuspicious =
    !Number.isFinite(renderedAt) || (formAgeMs >= 0 && formAgeMs < MIN_SUBMIT_MS);
