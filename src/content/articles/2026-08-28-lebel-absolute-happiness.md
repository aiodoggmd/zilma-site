---
title: "Lebel «Абсолютное счастье для волос»: программа восстановления IAU CELL CARE — состав, пропорции и техника нанесения"
description: "Салонная программа восстановления и увлажнения волос на молекулярном уровне: состав малого и большого набора, пошаговая техника нанесения, пропорции, время выдержки и домашний уход"
brand: "Lebel"
line: "Абсолютное счастье для волос"
tags: ["уход и восстановление"]
coverImage: "/images/articles/2026-08-28-lebel-absolute-happiness/cover.webp"
publishDate: 2026-08-28T12:00:00
verifiedDate: 2026-08-28
accent: "blue"
---

**Lebel «Абсолютное счастье для волос»** (Lebel Absolute Happiness for Hair) — концептуальная салонная программа восстановления и увлажнения волос на молекулярном уровне. Это не отдельное средство, а комплекс из четырёх линеек бренда (PROEDIT HAIRSKIN FLOAT CLEANSING, IAU CELL CARE, PROEDIT CARE WORKS, IAU ESSENCE), которые применяются последовательно за одну процедуру.

<div class="callout"><b>Новый взгляд на салонный уход.</b> Программа работает на молекулярном уровне, предлагая технологии восстановления кожи головы и волос. Это не разовая процедура «для блеска», а комплексный подход, который последовательно возвращает волосам здоровье, силу и увлажнение.</div>

<ul class="checklist">
  <li><span class="check-icon">✓</span>Натуральные волосы</li>
  <li><span class="check-icon">✓</span>Мелированные</li>
  <li><span class="check-icon">✓</span>Вьющиеся</li>
  <li><span class="check-icon">✓</span>Окрашенные</li>
</ul>

<div class="callout"><b>Курс и периодичность.</b> 3 этапа программы, от 3 до 7 применений на курс. При сильных повреждениях — раз в неделю, при более лёгких — раз в 2 недели.</div>

<p class="zoom-hint">Официальный сайт бренда (lebel.ru) отдельно уточняет: программу рекомендуется проводить за неделю до окрашивания и через 2 недели после него — это про оптимальный тайминг относительно процедуры окрашивания, а не запрет. Если нужно провести восстановление именно в день окрашивания — уточняйте у технолога бренда, насколько это критично для конкретного случая.</p>

## Состав программы

![Полный состав программы — сыворотки Proedit Care Works, мусс Float Cleansing и линейка IAU CELL CARE](/images/articles/2026-08-28-lebel-absolute-happiness/lineup.webp)

Программа поставляется в двух форматах — компактном салонном наборе и экономичном большом наборе в мягких упаковках.

<div class="callout"><b>Если какого-то товара Lebel нет в наличии,</b> напишите в комментариях — возможно оформление под заказ.</div>

### Малый набор (объёмы для салонного применения)

<div class="kit-table-wrap">
<table class="kit-table">
<colgroup><col class="col-num"><col class="col-name"><col class="col-art"><col class="col-order"></colgroup>
<thead><tr><th>№</th><th>Продукт</th><th>Артикул</th><th>Заказ</th></tr></thead>
<tbody>
<tr><td>1</td><td>Мусс для волос и кожи головы очищающий PROEDIT HAIRSKIN FLOAT CLEANSING, 145 г</td><td>3761</td><td class="col-order-cell"><button type="button" class="cart-add-btn" data-name="Мусс для волос и кожи головы очищающий PHFC 145 г. 3761" data-price="1857" data-brand="LEBEL">+ В заявку</button></td></tr>
<tr><td>2</td><td>Мусс клеточный увлажняющий IAU CELL CARE (1), 500 мл</td><td>5185</td><td class="col-order-cell"><button type="button" class="cart-add-btn" data-name="Мусс клеточный увлажняющий IAU CELL CARE (1) 500 мл. 5185" data-price="5282" data-brand="LEBEL">+ В заявку</button></td></tr>
<tr><td>3</td><td>Сыворотка PROEDIT CARE WORKS CMC (C), 150 мл</td><td>2849</td><td class="col-order-cell"><span class="order-unavailable">Под заказ</span></td></tr>
<tr><td>4</td><td>Сыворотка PROEDIT CARE WORKS NMF (N), 150 мл</td><td>2863</td><td class="col-order-cell"><span class="order-unavailable">Под заказ</span></td></tr>
<tr><td>5</td><td>Сыворотка PROEDIT CARE WORKS PPT (P), 150 мл</td><td>2856</td><td class="col-order-cell"><button type="button" class="cart-add-btn" data-name="№3 Сыв-ка д/волос PROEDIT CARE WORKS PPT 150 мл. 2856" data-price="2368" data-brand="LEBEL">+ В заявку</button></td></tr>
<tr><td>6</td><td>Сыворотка PROEDIT CARE WORKS ELEMENT FIX, 150 мл</td><td>2870</td><td class="col-order-cell"><button type="button" class="cart-add-btn" data-name="№4 Сыв-ка д/волос PROEDIT CARE WORKS ELEMENT FIX 150 мл. 2870" data-price="1986" data-brand="LEBEL">+ В заявку</button></td></tr>
<tr><td>7</td><td>Сыворотка протеиновая IAU CELL CARE (2), 150 мл</td><td>5208</td><td class="col-order-cell"><button type="button" class="cart-add-btn" data-name="Сыворотка протеиновая IAU Cell Care 2, 150 мл 5208" data-price="3625" data-brand="LEBEL">+ В заявку</button></td></tr>
<tr><td>8</td><td>Крем интенсивный для укрепления IAU CELL CARE (3S), 500 мл</td><td>5222</td><td class="col-order-cell"><button type="button" class="cart-add-btn" data-name="Крем интенс. д/укр. волос IAU CELL CARE 3S 500мл 5222" data-price="5282" data-brand="LEBEL">+ В заявку</button></td></tr>
<tr><td>9</td><td>Крем интенсивный для увлажнения IAU CELL CARE (3M), 500 мл</td><td>5246</td><td class="col-order-cell"><button type="button" class="cart-add-btn" data-name="Крем интенсивный для увлажнения волос IAU CELL CARE 3М 500мл 5246" data-price="5283" data-brand="LEBEL">+ В заявку</button></td></tr>
<tr><td>10</td><td>Гель фиксирующий IAU CELL CARE (4), 500 мл</td><td>5260</td><td class="col-order-cell"><button type="button" class="cart-add-btn" data-name="Гель фиксирующий IAU CELL CARE (4)500ml 5260" data-price="5282" data-brand="LEBEL">+ В заявку</button></td></tr>
</tbody>
</table>
</div>

### Большой набор (мягкие упаковки, экономичный формат)

<div class="kit-table-wrap">
<table class="kit-table">
<colgroup><col class="col-num"><col class="col-name"><col class="col-art"><col class="col-order"></colgroup>
<thead><tr><th>№</th><th>Продукт</th><th>Артикул</th><th>Заказ</th></tr></thead>
<tbody>
<tr><td>1</td><td>Мусс очищающий PROEDIT HAIRSKIN FLOAT CLEANSING, 250 г</td><td>4058</td><td class="col-order-cell"><button type="button" class="cart-add-btn" data-name="Мусс для волос и кожи головы очищающий PHFC 250 г. 4058" data-price="2616" data-brand="LEBEL">+ В заявку</button></td></tr>
<tr><td>2</td><td>Мусс клеточный увлажняющий IAU CELL CARE (1), 1000 мл</td><td>5192</td><td class="col-order-cell"><span class="order-unavailable">Под заказ</span></td></tr>
<tr><td>3</td><td>Сыворотка CMC (C), 500 мл</td><td>3372</td><td class="col-order-cell"><button type="button" class="cart-add-btn" data-name="№1 Сыв-ка д/волос PROEDIT CARE WORKS CMC 500 мл. 3372" data-price="5176" data-brand="LEBEL">+ В заявку</button></td></tr>
<tr><td>4</td><td>Сыворотка NMF (N), 500 мл</td><td>3389</td><td class="col-order-cell"><button type="button" class="cart-add-btn" data-name="№2 Сыв-ка д/волос PROEDIT CARE WORKS NMF 500 мл. 3389" data-price="4528" data-brand="LEBEL">+ В заявку</button></td></tr>
<tr><td>5</td><td>Сыворотка PPT (P), 500 мл</td><td>3396</td><td class="col-order-cell"><button type="button" class="cart-add-btn" data-name="№3 Сыв-ка д/волос PROEDIT CARE WORKS PPT 500мл 3396" data-price="5176" data-brand="LEBEL">+ В заявку</button></td></tr>
<tr><td>6</td><td>Сыворотка ELEMENT FIX, 500 мл</td><td>3402</td><td class="col-order-cell"><button type="button" class="cart-add-btn" data-name="№4 Сыв-ка д/волос PROEDIT CARE WORKS ELEMENT FIX 500 мл. 3402" data-price="4528" data-brand="LEBEL">+ В заявку</button></td></tr>
<tr><td>7</td><td>Сыворотка протеиновая IAU CELL CARE (2), 500 мл</td><td>5215</td><td class="col-order-cell"><button type="button" class="cart-add-btn" data-name="Сыворотка протеиновая IAU Cell Care 2, 500 мл 5215" data-price="7844" data-brand="LEBEL">+ В заявку</button></td></tr>
<tr><td>8</td><td>Крем укрепляющий IAU CELL CARE (3S), 1000 мл</td><td>5239</td><td class="col-order-cell"><button type="button" class="cart-add-btn" data-name="Крем интенс. д/укр. волос IAU CELL CARE 3S 1000мл 5239" data-price="8415" data-brand="LEBEL">+ В заявку</button></td></tr>
<tr><td>9</td><td>Крем увлажняющий IAU CELL CARE (3M), 1000 мл</td><td>5253</td><td class="col-order-cell"><button type="button" class="cart-add-btn" data-name="Крем интенсивный для увлажнения волос IAU CELL CARE 3М 1000мл 5253" data-price="8415" data-brand="LEBEL">+ В заявку</button></td></tr>
<tr><td>10</td><td>Гель фиксирующий IAU CELL CARE (4), 1000 мл</td><td>5277</td><td class="col-order-cell"><span class="order-unavailable">Под заказ</span></td></tr>
</tbody>
</table>
</div>

## Способ применения

### Шаг 1. Очищение

Вымойте волосы подходящим шампунем. Нанесите мусс PROEDIT HAIRSKIN FLOAT CLEANSING на кожу головы и волосы, помассируйте 7–8 минут, тщательно смойте водой.

<div class="timer-chip"><span class="wheel sm" style="--split: 100%"></span><b>7–8 минут</b> массажа кожи головы перед смыванием.</div>

### Шаг 2. Клеточное увлажнение

<div class="mix-card">
  <div class="wheel-badge">
    <span class="wheel" style="--split: 33%"></span>
    <span class="wheel-label">1 : 2</span>
    <span class="wheel-caption">IAU CELL CARE (1) : вода</span>
  </div>
  <div class="mix-body">
    <p>Налейте в чашу для взбивания горячую воду и средство IAU CELL CARE (1) в пропорции 1:2. На длинные волосы — 8 нажатий дозатора, на средние — 6 нажатий (1 нажатие ≈ 3 мл). Вспеньте, нанесите на кожу головы, массируйте 5 пальцами по секторам, остаток распределите по длине. Тщательно смойте.</p>
  </div>
</div>

### Шаг 3. Нанесение сывороток

![Сыворотки PROEDIT CARE WORKS — C, N, P и Element Fix](/images/articles/2026-08-28-lebel-absolute-happiness/careworks-row.webp)

Промокните волосы полотенцем. Разделите волосы от макушки на 6–8 зон. Отступите 1–2 см от корней и нанесите сыворотки последовательно: **C → N → P → Element Fix**. Наденьте полиэтиленовую шапочку.

<div class="timer-chip"><span class="wheel sm" style="--split: 100%"></span>Выдержка <b>5–15 минут, без нагрева</b></div>

### Шаг 4. Протеиновое укрепление

Не смывая предыдущий слой, нанесите протеиновую сыворотку IAU CELL CARE (2) попрядно, прорабатывая гребнеобразными движениями. Время выдержки не требуется — сразу переходите к следующему этапу.

### Шаг 5. Интенсивный крем

Не смывая предыдущий этап, нанесите крем IAU CELL CARE (3M) на пряди для увлажнения. Для сильно повреждённых волос добавьте сразу и IAU CELL CARE (3S) для укрепления — оба без смывания и без дополнительной выдержки.

### Шаг 6. Фиксация и завершение

Нанесите гель IAU CELL CARE (4) гребнеобразными движениями. Без выдержки выполните технику горячих полотенец, затем тщательно смойте весь комплекс водой.

### Шаг 7. Завершающий уход

![IAU Essence — Sleek, Moist и Forti](/images/articles/2026-08-28-lebel-absolute-happiness/essence-lineup.webp)

Нанесите IAU ESSENCE — MOIST, SLEEK или FORTI, в зависимости от желаемого результата — и приступайте к укладке.

<div class="callout">
  <p class="callout-title">MOIST</p>
  <span class="callout-sub">Увлажняющее молочко, 100 мл · арт. 4676</span>
  <ul>
    <li>Для глубокого увлажнения пористой, обезвоженной структуры волос</li>
    <li>Сохраняет молодость волос, препятствует истончению структуры</li>
    <li>Защищает от агрессивных факторов окружающей среды и термовоздействия</li>
    <li>Придаёт мягкость, упругость, шелковистость, эластичность</li>
    <li>Обволакивает волосы, облегчает расчёсывание</li>
    <li>Препятствует впитыванию неприятных запахов из внешней среды</li>
    <li><b>SPF 30</b></li>
  </ul>
  <p><b>Активные ингредиенты:</b> CMC-комплекс, масло авокадо, глицерин, масло ши, экстракт тремеллы (ледяной гриб).</p>
  <div class="callout-order"><button type="button" class="cart-add-btn" data-name="Эссенция &quot;Moist&quot; увлажняющее молочко, 100 мл. 4676-3884" data-price="3401" data-brand="LEBEL">+ В заявку</button></div>
</div>

<div class="callout">
  <p class="callout-title">SLEEK</p>
  <span class="callout-sub">Разглаживающее масло, 100 мл · арт. 4669</span>
  <ul>
    <li>Убирает пушистость и разглаживает внешнюю структуру волос</li>
    <li>Сохраняет молодость волос, препятствует истончению структуры</li>
    <li>Восстанавливает гидролипидный баланс</li>
    <li>Придаёт глянцевый блеск</li>
    <li>Обволакивает волос, облегчая расчёсывание и укладку</li>
    <li>Защищает от агрессивной среды, термического воздействия и УФ-лучей</li>
    <li>Препятствует впитыванию неприятных внешних запахов</li>
  </ul>
  <p><b>Активные ингредиенты:</b> CMC-комплекс, масло жожоба, масло авокадо, масло ши.</p>
  <div class="callout-order"><span class="order-unavailable">Под заказ</span></div>
</div>

<div class="callout">
  <p class="callout-title">FORTI</p>
  <span class="callout-sub">Укрепляющий гель, 100 мл · арт. 4683</span>
  <ul>
    <li>Восстанавливает и укрепляет структуру тонких и ослабленных волос</li>
    <li>Сохраняет молодость волос, препятствует истончению структуры</li>
    <li>Придаёт эластичность, шелковистость, плотность и блеск</li>
    <li>Защищает от агрессивных факторов среды и УФ-лучей</li>
    <li>Препятствует образованию статики</li>
    <li>Препятствует впитыванию неприятных внешних запахов</li>
    <li><b>SPF 30</b></li>
  </ul>
  <p><b>Активные ингредиенты:</b> CMC-комплекс, гидролизованный кератин, масло авокадо, глицерин, масло ши.</p>
  <div class="callout-order"><span class="order-unavailable">Под заказ</span></div>
</div>

<div class="service-card">
  <div>
    <p class="service-title">Способ применения <span class="ratio-tag">общий для всех трёх</span></p>
    <p>Равномерно распределите эссенцию по влажным волосам, начиная с кончиков, по всей длине (2 нажатия дозатора для волос средней длины). Приступайте к укладке. Для дополнительного блеска нанесите немного средства (1 нажатие) уже на высушенные волосы.</p>
  </div>
</div>

## Домашний уход

![IAU CELL CARE 5M и 5S — крема-концентраты для домашнего ухода](/images/articles/2026-08-28-lebel-absolute-happiness/cellcare-5m-5s.webp)

<div class="kit-table-wrap">
<table class="kit-table">
<colgroup><col class="col-name"><col class="col-art"><col class="col-order"></colgroup>
<thead><tr><th>Продукт</th><th>Артикул</th><th>Заказ</th></tr></thead>
<tbody>
<tr><td>Крем-концентрат для увлажнения волос IAU CELL CARE 5M, 40 мл</td><td>5291</td><td class="col-order-cell"><span class="order-unavailable">Под заказ</span></td></tr>
<tr><td>Крем-концентрат для укрепления волос IAU CELL CARE 5S, 40 мл</td><td>5284</td><td class="col-order-cell"><span class="order-unavailable">Под заказ</span></td></tr>
</tbody>
</table>
</div>

Для поддержания результата между процедурами: вымойте волосы шампунем, нанесите крем IAU CELL CARE (5M) на пряди, проработайте гребнеобразными движениями. Для сильных повреждений добавьте также крем IAU CELL CARE (5S) — без смывания и выдержки.

<div class="timer-chip"><span class="wheel sm" style="--split: 100%"></span>Выдержите <b>5 минут</b>, смойте водой.</div>

## Важное примечание

<div class="callout"><b>Дозаторы продаются отдельно.</b> Сыворотки C, N, P, ELEMENT FIX и все продукты в мягких упаковках поставляются без дозаторов.</div>

<div class="kit-table-wrap">
<table class="kit-table">
<colgroup><col class="col-name"><col class="col-art"><col class="col-order"></colgroup>
<thead><tr><th>Дозатор</th><th>Артикул</th><th>Заказ</th></tr></thead>
<tbody>
<tr><td>Дозатор для сывороток C, N, P (150 мл)</td><td>6052</td><td class="col-order-cell"><button type="button" class="cart-add-btn" data-name="Дозатор - Распылитель для спрея 24*180 мм Proedit Care Works 6052/8149" data-price="85" data-brand="LEBEL">+ В заявку</button></td></tr>
<tr><td>Дозатор для сыворотки ELEMENT FIX (150 мл)</td><td>6050</td><td class="col-order-cell"><button type="button" class="cart-add-btn" data-name="Дозатор - носик, насадка на бутылку Proedit Care Works (element) 6050" data-price="85" data-brand="LEBEL">+ В заявку</button></td></tr>
<tr><td>Дозатор для мягких упаковок</td><td>3876</td><td class="col-order-cell"><button type="button" class="cart-add-btn" data-name="Дозатор (длинный) для шампуня/масок в мягких пакетах (33/410) 1000мл 3876лр" data-price="127.5" data-brand="LEBEL">+ В заявку</button></td></tr>
<tr><td>Корпус для мягких упаковок</td><td>1725</td><td class="col-order-cell"><button type="button" class="cart-add-btn" data-name="Корпус пласт. для мягкой упаковки (уход) 1000мл Lebel ONE 1725лр" data-price="246.5" data-brand="LEBEL">+ В заявку</button></td></tr>
</tbody>
</table>
</div>
