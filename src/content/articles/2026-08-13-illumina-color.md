---
title: "Wella Illumina Color: технология MICROLIGHT, пропорции смешивания и Opal-Essence"
seoTitle: "Иллюмина (Illumina): как смешивать, пропорции и оксид"
description: "Иллюмину смешивают с Welloxon Perfect 1:1. Выдержка 30–40 минут без тепла, 15–25 с теплом. Оксид 4–12% под задачу, на седину — 6%."
answer: "Illumina Color смешивают с Welloxon Perfect 1:1 — например, 60 мл краски и 60 мл окислителя; выдержка 30–40 минут без тепла или 15–25 минут с теплом. Окислитель: 4% — более глубокий результат без осветления, 6% — тон в тон или темнее и закрашивание седины, 9% — осветление на 2 уровня, 12% — на 3. При седине 70–100% оттенок смешивают 1:1 с нейтральным той же глубины на Welloxon Perfect 6%."
brand: "Wella"
line: "Illumina Color"
tags: ["колористика"]
coverImage: "/images/articles/2026-08-13-illumina-color/cover.webp"
publishDate: 2026-08-13T08:00:00
verifiedDate: 2026-09-25
---

**Illumina Color** — перманентная крем-краска Wella Professionals на технологии MICROLIGHT: максимально естественный результат окрашивания с сиянием, заметным при любом освещении. Руководство по применению: покрытие седины, схема смешивания с Welloxon Perfect, область применения и отдельная линейка металлик-оттенков Opal-Essence.

![Illumina Color — краска и линейка оттенков](/images/articles/2026-08-13-illumina-color/hero-product.webp)

## Как смешивать Иллюмину с окислителем

Illumina Color смешивают **1:1** с Welloxon Perfect — например, 60 мл краски и 60 мл окислителя. Выдержка при окрашивании — **30–40 минут без тепла** или **15–25 минут с теплом**. При осветлении время другое: 20 минут без тепла и 10 минут с теплом для деликатного результата, 30–40 и 15–25 минут — для более интенсивного.

**Смешивание на седину.** До 70% седины оттенок Illumina Color используют как есть. При седине 70–100% нужный оттенок смешивают 1:1 с нейтральным оттенком той же глубины и добавляют Welloxon Perfect 6%. Пример из инструкции: 30 г 7/ + 30 г 7/35 + 60 г Welloxon Perfect 6%.

**Какой процент.** 4% — более глубокий результат на натуральных волосах без осветления. 6% — тот же тон или на уровень темнее, закрашивание седины. 9% — осветление на 2 уровня. 12% — на 3 уровня, это максимум для линейки. С Koleston Perfect смешивать Illumina Color не рекомендуется.

## Что такое Illumina Color

Стойкая крем-краска, которая позволяет осветлить, затемнить или подчеркнуть натуральный цвет волос для создания естественного образа.

- Покрытие седины до 100%, гарантированный результат.
- Чистый цвет, сияющий изнутри при любом освещении — хороший первый опыт для клиентов, которые боятся окрашивания волос.
- До трёх уровней осветления.
- Палитра из 49 оттенков, включая четыре оттенка серии Warm Shimmer (9/37, 8/36, 7/42, 7/75). Все оттенки Illumina Color можно смешивать друг с другом, создавая индивидуальную палитру — представлены тёплые, нейтральные и холодные направления.

<div class="callout"><b>Технология MICROLIGHT.</b> Защищает кутикулу волоса, не перегружая её, — лучи света свободно проникают внутрь волоса, придавая естественным переливам цвета сияние, заметное при любом освещении. На поверхности кутикулы присутствуют частицы меди: они вступают в реакцию со стандартной красящей смесью, оставляя поверхность волоса жёсткой и грубой, из-за чего лучи света рассеиваются, а не отражаются. MICROLIGHT обволакивает медные частицы, препятствуя их взаимодействию с красящей смесью, и тем самым повышает защиту чешуйчатого слоя. В паре с MICROLIGHT работает комплекс <b>Metal Purifier</b> — связывает ионы металлов, накопленные волосом от воды и предыдущих окрашиваний, и не даёт им искажать цвет или ослаблять волос.</div>

## Схема смешивания с Welloxon Perfect

<div class="mix-card">
  <div class="wheel-badge">
    <span class="wheel" style="--split: 50%"></span>
    <span class="wheel-label">1 : 1</span>
    <span class="wheel-caption">Illumina · Welloxon</span>
  </div>
  <div class="mix-body">
    <p><b>Базовое окрашивание.</b> Пример: 60 мл Illumina Color + 60 мл Welloxon Perfect. Процент окислителя — по задаче:</p>
    <ul>
      <li><b>4%</b> — более глубокий результат на натуральных волосах без осветления</li>
      <li><b>6%</b> — тот же тон или на 1 уровень темнее, закрашивание седины</li>
      <li><b>9%</b> — осветление на 2 уровня</li>
      <li><b>12%</b> — осветление на 3 уровня (максимум для линии)</li>
    </ul>
    <p>Не рекомендуется смешивать с Koleston Perfect.</p>
  </div>
  <div class="timer">
    <span class="wheel" style="--split: 67%"></span>
    <span class="time-text">30–40 мин <b>без тепла</b>, 15–25 мин <b>с теплом</b></span>
  </div>
</div>

<div class="callout">При <b>осветлении</b> время выдержки другое: <b>20 мин без тепла / 10 мин с теплом</b> — для деликатного осветления, <b>30–40 мин без тепла / 15–25 мин с теплом</b> — для более интенсивного эффекта.</div>

<div class="kit-table-wrap">
<table class="kit-table">
<colgroup><col><col style="width:90px"><col style="width:130px"><col style="width:108px"></colgroup>
<thead><tr><th>Продукт</th><th>Артикул</th><th>Цена</th><th>Заказ</th></tr></thead>
<tbody>
<tr><td>Окислитель Welloxon Perfect 1,9%, 1000 мл</td><td>6495</td><td class="col-price-cell"><s class="kit-old-price">998,58 ₽</s><b class="kit-price-now">928,68 ₽</b><span class="kit-promo-badge">Акция</span></td><td class="col-order-cell"><button type="button" class="cart-add-btn" data-name="Окислитель Welloxon Perfect 1,9% 1000мл ДЛЯ ПАСТЕЛЬНОГО ТОНИРОВАНИЯ 6495/6921" data-price="928.68" data-brand="WELLA">+ В заявку</button></td></tr>
<tr><td>Окислитель Welloxon Perfect 4%, 1000 мл</td><td>6496</td><td class="col-price-cell"><s class="kit-old-price">998,58 ₽</s><b class="kit-price-now">928,68 ₽</b><span class="kit-promo-badge">Акция</span></td><td class="col-order-cell"><button type="button" class="cart-add-btn" data-name="Окислитель Welloxon Perfect 4% 1000мл ДЛЯ ПАСТЕЛЬНОГО ТОНИРОВАНИЯ 6496" data-price="928.68" data-brand="WELLA">+ В заявку</button></td></tr>
<tr><td>Окислитель Welloxon Perfect 6%, 1000 мл</td><td>6497</td><td class="col-price-cell"><s class="kit-old-price">998,58 ₽</s><b class="kit-price-now">928,68 ₽</b><span class="kit-promo-badge">Акция</span></td><td class="col-order-cell"><button type="button" class="cart-add-btn" data-name="Окислитель Welloxon Perfect 6% 1000мл 6497/6923" data-price="928.68" data-brand="WELLA">+ В заявку</button></td></tr>
<tr><td>Окислитель Welloxon Perfect 9%, 1000 мл</td><td>6498</td><td class="col-price-cell"><s class="kit-old-price">998,58 ₽</s><b class="kit-price-now">928,68 ₽</b><span class="kit-promo-badge">Акция</span></td><td class="col-order-cell"><button type="button" class="cart-add-btn" data-name="Окислитель Welloxon Perfect 9% 1000 мл. 6498/6484" data-price="928.68" data-brand="WELLA">+ В заявку</button></td></tr>
<tr><td>Окислитель Welloxon Perfect 12%, 1000 мл</td><td>6499</td><td class="col-price-cell"><s class="kit-old-price">998,58 ₽</s><b class="kit-price-now">928,68 ₽</b><span class="kit-promo-badge">Акция</span></td><td class="col-order-cell"><button type="button" class="cart-add-btn" data-name="Окислитель Welloxon Perfect 12% 1000мл 6499/6488" data-price="928.68" data-brand="WELLA">+ В заявку</button></td></tr>
<tr><td>Окислитель Welloxon Perfect 12%, 60 мл (пробник/докупка)</td><td>6494</td><td class="col-price-cell"><s class="kit-old-price">194 ₽</s><b class="kit-price-now">180,42 ₽</b><span class="kit-promo-badge">Акция</span></td><td class="col-order-cell"><button type="button" class="cart-add-btn" data-name="Окислитель Welloxon Perfect 12% 60мл 6494/50931" data-price="180.42" data-brand="WELLA">+ В заявку</button></td></tr>
</tbody>
</table>
</div>

**Закрашивание седины:**

- **До 70% седины** — использовать любой оттенок Illumina Color как есть.
- **70–100% седины** — смешать 1:1 нужный оттенок с соответствующим нейтральным оттенком той же глубины + Welloxon Perfect 6%. Например: 30 г 7/ + 30 г 7/35 + 60 г Welloxon Perfect 6%/20 vol.

## Область применения

Illumina Color подходит для:

- окрашивания всей массы волос, частичного и мультитонального окрашивания;
- осветления, затемнения и тонирования тон в тон;
- ламинирования;
- коррекции цвета;
- освежения цвета и выравнивания тона.

## Opal-Essence by Illumina Color

![Opal-Essence by Illumina Color — четыре металлик-оттенка](/images/articles/2026-08-13-illumina-color/variant-opal-essence.webp)

**Opal-Essence** — группа из четырёх металлик-оттенков внутри линейки Illumina Color, без глубины тона. Изменение цвета достигается через движение и свет: мягкое сияние с одновременно тёплыми и холодными переливами. Оттенки — **Лиловое серебро, Платиновая лилия, Титановый розовый, Медный персик**.

### Как использовать

<ul class="frac-list">
  <li><span class="wheel sm" style="--split: 100%"></span><div><b>Самостоятельно</b> — оттенок Opal-Essence наносится без добавления других красок</div></li>
  <li><span class="wheel sm" style="--split: 33%"></span><div><b>В сочетании с Illumina Color</b> — например, 1 часть оттенка 10/05 + 2 части Opal-Essence Titanium Rose</div></li>
</ul>

<div class="mix-card">
  <div class="wheel-badge">
    <span class="wheel" style="--split: 50%"></span>
    <span class="wheel-label">1 : 1</span>
    <span class="wheel-caption">Opal-Essence · Welloxon</span>
  </div>
  <div class="mix-body">
    <p>Максимальный результат достигается с низким процентом окислителя — смешать 1:1.</p>
    <ul>
      <li><b>1,9%</b> — для первичного окрашивания</li>
      <li><b>4%</b> — первичное окрашивание, окрашивание прикорневой зоны с максимально выраженным металлик-эффектом</li>
      <li><b>6%</b> — креативное применение в сочетании с Illumina Color</li>
      <li><b>9%</b> — не рекомендуется! Металлик-эффект будет потерян</li>
    </ul>
  </div>
  <div class="timer">
    <span class="wheel" style="--split: 65%"></span>
    <span class="time-text">30–40 мин <b>без тепла</b></span>
  </div>
</div>

<div class="callout"><b>Покрытие седины.</b> Opal-Essence даёт переливающийся металлик-эффект без покрытия седины.</div>

## После процедуры

Слегка вымыть волосы шампунем для окрашенных волос (шампунь Brilliance — в таблице ниже), затем нанести стабилизатор для сохранения цвета — техническая замена на <a href="/articles/2026-08-29-londa-professional/#завершение-процедуры">стабилизатор Londa</a>.

В прайсе Zilma для этих шагов доступны:

<div class="kit-table-wrap">
<table class="kit-table">
<colgroup><col class="col-num"><col class="col-name"><col class="col-art"><col class="col-price"><col class="col-order"></colgroup>
<thead><tr><th>№</th><th>Продукт</th><th>Артикул</th><th>Цена</th><th>Заказ</th></tr></thead>
<tbody>
<tr><td>1</td><td>Brilliance Бальзам-уход для нормальных и тонких волос, 1000 мл</td><td>339306</td><td class="col-price-cell">2824.55 ₽</td><td class="col-order-cell"><button type="button" class="cart-add-btn" data-name="Brilliance Бальзам-уход д/защиты цвета окр.норм.и тонких волос 1000мл 339306" data-price="2824.55" data-brand="WELLA">+ В заявку</button></td></tr>
<tr><td>2</td><td>Brilliance Бальзам-уход для жёстких волос, 200 мл</td><td>339252</td><td class="col-price-cell">998.75 ₽</td><td class="col-order-cell"><button type="button" class="cart-add-btn" data-name="Brilliance Бальзам-уход д/защиты цвета окр.жестк.волос 200мл 339252" data-price="998.75" data-brand="WELLA">+ В заявку</button></td></tr>
<tr><td>3</td><td>Brilliance Бальзам-уход для жёстких волос, 1000 мл</td><td>339313</td><td class="col-price-cell">2824.55 ₽</td><td class="col-order-cell"><button type="button" class="cart-add-btn" data-name="Brilliance Бальзам-уход д/защиты цвета окр.жестк.волос 1000мл 339313" data-price="2824.55" data-brand="WELLA">+ В заявку</button></td></tr>
<tr><td>4</td><td>Brilliance Шампунь для нормальных и тонких волос, 300 мл</td><td>339221</td><td class="col-price-cell">919.7 ₽</td><td class="col-order-cell"><button type="button" class="cart-add-btn" data-name="Brilliance Шампунь д/защиты цвета окр.норм. и тонких волос 300мл 339221" data-price="919.7" data-brand="WELLA">+ В заявку</button></td></tr>
<tr><td>5</td><td>Brilliance Шампунь для нормальных и тонких волос, 1000 мл</td><td>339283</td><td class="col-price-cell">2047.48 ₽</td><td class="col-order-cell"><button type="button" class="cart-add-btn" data-name="Brilliance Шампунь д/защиты цвета окр.норм. и тонких волос 1000мл 339283" data-price="2047.48" data-brand="WELLA">+ В заявку</button></td></tr>
<tr><td>6</td><td>Brilliance Шампунь для жёстких волос, 300 мл</td><td>339238</td><td class="col-price-cell">919.7 ₽</td><td class="col-order-cell"><button type="button" class="cart-add-btn" data-name="Brilliance Шампунь д/защиты цвета окр.жестк.волос 300мл 339238" data-price="919.7" data-brand="WELLA">+ В заявку</button></td></tr>
<tr><td>7</td><td>Brilliance Шампунь для жёстких волос, 1000 мл</td><td>339290</td><td class="col-price-cell">2047.48 ₽</td><td class="col-order-cell"><button type="button" class="cart-add-btn" data-name="Brilliance Шампунь д/защиты цвета окр.жестк.волос 1000мл 339290" data-price="2047.48" data-brand="WELLA">+ В заявку</button></td></tr>
</tbody>
</table>
</div>

## Палитра оттенков

Палитра включает тёплые и нейтральные оттенки (глубина тона 4/–10/), холодные оттенки той же шкалы и четыре металлик-оттенка Opal-Essence (Silver Mauve, Platinum Lily, Titanium Rose, Copper Peach).
