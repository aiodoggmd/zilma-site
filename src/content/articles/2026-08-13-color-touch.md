---
title: "Wella Color Touch: интенсивное деми-перманентное тонирование без аммиака"
seoTitle: "Колор Тач (Color Touch): как разводить и сколько держать"
description: "Колор Тач разводят с эмульсией 1:2 — 1,9% или 4%. Выдержка 20 минут без тепла, 15 с теплом. Седина: базовый Color Touch до 50%, Plus до 70%."
answer: "Color Touch разводят 1:2 — одна часть краски на две части эмульсии Color Touch 1,9% или 4%; выдержка 20 минут без тепла или 15 минут с теплом. Так же разводят Color Touch Plus. Эмульсия 1,9% — тонирование тон в тон, 4% — плотнее по седине и ярче. Базовый Color Touch закрашивает до 50% седины, Plus — до 70%."
brand: "Wella"
line: "Color Touch"
tags: ["колористика"]
coverImage: "/images/articles/2026-08-13-color-touch/cover.webp"
publishDate: 2026-08-13T07:00:00
verifiedDate: 2026-09-25
---

**Color Touch** — интенсивное тонирование. Color Touch открывает вашим клиентам мир модного цвета: блеск и яркость стойкого результата с мягким воздействием, характерным для полуперманентной краски.

![Color Touch — флакон эмульсии и коробки с краской](/images/articles/2026-08-13-color-touch/hero-product.webp)

- Без аммиака.
- Яркий блеск для великолепных модных результатов.
- Простота ухода, почти незаметное отрастание корней.

## Как разводить Колор Тач и сколько держать

Color Touch разводят **1:2**: одна часть краски на две части эмульсии Color Touch 1,9% или 4%. Выдержка — **20 минут без тепла** или **15 минут с теплом**. Соотношение одинаково для всей линейки: так же разводят Color Touch Plus и каждую формулу в технике Color Touch Melt.

**Седина.** Базовый Color Touch закрашивает до 50% седых волос. Color Touch Plus закрашивает до 70% и работает на эмульсии 4%.

**Какую эмульсию выбрать.** У линейки своя эмульсия Color Touch 1,9% и 4%; официальная инструкция Wella привязывает краску именно к ней. **1,9%** берут для чистого тонирования тон в тон, без осветления. **4%** — когда нужна плотность по седине и больше яркости в цвете; на ней же работает Color Touch Plus. На одинаковом проценте окислители технически близки, поэтому равнозначная замена по проценту допустима.

## Что это

- Интенсивное тонирование.
- Закрашивает до 70% седых волос (Color Touch Plus).
- Стойкость до 28 раз мытья шампунем.
- Яркий блеск для великолепных модных результатов.
- Более 90 оттенков — для неограниченного количества вариантов цвета.
- Веганская формула без силиконов.
- Без обязательств, без аммиака, многогранный цвет волос для клиентов, которые любят регулярно менять цвет волос.
- Идеально подходит для творчества, улучшения цвета, добавления глубины и тона.

## Color Touch

<div class="service-card">
  <span class="wheel" style="--split: 33%"></span>
  <div>
    <p class="service-title">Color Touch <span class="ratio-tag">1:2</span></p>
    <p>1 часть Color Touch + 2 части эмульсии Color Touch 1,9% / 4%. Технология — комплекс Metal Purifier.</p>
    <p class="service-time">Время выдержки: <b>20 мин без тепла</b> / <b>15 мин с теплом</b></p>
  </div>
</div>

**Закрашивание седины:** до 50%.

**Область применения:**

<ul class="checklist">
  <li><span class="check-icon">✓</span>Окрашивание всей массы волос / Частичное окрашивание / Мультитональное окрашивание</li>
  <li><span class="check-icon">✓</span>Темнее / Тон в тон</li>
  <li><span class="check-icon">✓</span>Ламинирование Crystal Gloss</li>
  <li><span class="check-icon">✓</span>Коррекция цвета</li>
  <li><span class="check-icon">✓</span>Освежение цвета / Выравнивание цвета</li>
</ul>

<div class="kit-table-wrap">
<table class="kit-table">
<colgroup><col><col style="width:90px"><col style="width:130px"><col style="width:108px"></colgroup>
<thead><tr><th>Продукт</th><th>Артикул</th><th>Цена</th><th>Заказ</th></tr></thead>
<tbody>
<tr><td>Оксид Color Touch 1,9%, 1000 мл</td><td>6021</td><td class="col-price-cell">998,58 ₽</td><td class="col-order-cell"><button type="button" class="cart-add-btn" data-name="Оксид Color Touch 1,9 % 1000 мл. 6021/6298" data-price="998.58" data-brand="WELLA">+ В заявку</button></td></tr>
<tr><td>Оксид Color Touch 4%, 1000 мл</td><td>6019</td><td class="col-price-cell"><s class="kit-old-price">998,58 ₽</s><b class="kit-price-now">928,68 ₽</b><span class="kit-promo-badge">Акция</span></td><td class="col-order-cell"><button type="button" class="cart-add-btn" data-name="Оксид Color Touch 4 % 1000 мл. 6019/6291" data-price="928.68" data-brand="WELLA">+ В заявку</button></td></tr>
</tbody>
</table>
</div>

<div class="callout"><b>О названиях окислителей.</b> Официальная инструкция Wella привязывает каждый продукт линейки к своему фирменному окислителю (Welloxon Perfect, «Оксид Color Touch», окислитель Freelights). На одинаковом проценте они технически близки, как и <a href="/articles/2026-08-29-londa-professional/#окислитель-l-color">окислительная эмульсия Londa L-Color</a> — равнозначная замена по проценту технически допустима.</div>

## Color Touch Plus

![Color Touch Plus](/images/articles/2026-08-13-color-touch/variant-plus.webp)

<div class="service-card">
  <span class="wheel" style="--split: 33%"></span>
  <div>
    <p class="service-title">Color Touch Plus <span class="ratio-tag">1:2</span></p>
    <p><b>Результат:</b> закрашивает на 20% больше седых волос по сравнению с Color Touch — для клиентов, которые не готовы к стойкой краске для волос. 1 часть Color Touch Plus + 2 части эмульсии Color Touch Plus 4%. Технология — комплекс Metal Purifier, идеально для интенсивных насыщенных оттенков.</p>
    <p class="service-time">Время выдержки: <b>20 мин без тепла</b> / <b>15 мин с теплом</b></p>
  </div>
</div>

**Закрашивание седины:** до 70%.

**Область применения:**

<ul class="checklist">
  <li><span class="check-icon">✓</span>Окрашивание всей массы волос / Частичное окрашивание / Мультитональное окрашивание</li>
  <li><span class="check-icon">✓</span>Темнее / Тон в тон</li>
  <li><span class="check-icon">✓</span>Ламинирование Crystal Gloss</li>
  <li><span class="check-icon">✓</span>Коррекция цвета</li>
  <li><span class="check-icon">✓</span>Освежение цвета / Выравнивание цвета</li>
</ul>

## Color Touch Melt

Фирменная техника многотонового окрашивания: несколько формул Color Touch наносятся зонами (растяжка цвета), а в последние 5 минут выдержки цвета смешиваются гребнем по всей длине волос от корней к кончикам для бесшовного перехода.

<div class="service-card">
  <span class="wheel" style="--split: 33%"></span>
  <div>
    <p class="service-title">Color Touch Melt <span class="ratio-tag">1:2</span></p>
    <p>Каждая формула — 1 часть Color Touch + 2 части эмульсии Color Touch 1,9% / 4%. За 5 минут до конца выдержки цвета растушёвываются гребнем.</p>
    <p class="service-time">Время выдержки: <b>20 мин без тепла</b> / <b>15 мин с теплом</b></p>
  </div>
</div>

## После процедуры

Слегка вымыть волосы шампунем для окрашенных волос (шампунь Brilliance — в таблице ниже), затем нанести стабилизатор для сохранения цвета — техническая замена на <a href="/articles/2026-08-29-londa-professional/#завершение-процедуры">стабилизатор Londa</a>.

В прайсе Zilma для этих шагов доступны:

<div class="kit-table-wrap">
<table class="kit-table">
<colgroup><col class="col-num"><col class="col-name"><col class="col-art"><col class="col-price"><col class="col-order"></colgroup>
<thead><tr><th>№</th><th>Продукт</th><th>Артикул</th><th>Цена</th><th>Заказ</th></tr></thead>
<tbody>
<tr><td>1</td><td>Brilliance Бальзам-уход для нормальных и тонких волос, 1000 мл</td><td>339306</td><td class="col-price-cell">2824.55 ₽</td><td class="col-order-cell"><button type="button" class="cart-add-btn" data-name="Brilliance Бальзам-уход д/защиты цвета окр.норм.и тонких волос 1000мл 339306" data-price="2824.55" data-brand="WELLA">+ В заявку</button></td></tr>
<tr><td>2</td><td>Brilliance Бальзам-уход для жёстких волос, 200 мл</td><td>339252</td><td class="col-price-cell">998.75 ₽</td><td class="col-order-cell"><button type="button" class="cart-add-btn" data-name="Brilliance Бальзам-уход д/защиты цвета окр.жестк.волос 200мл 339252" data-price="998.75" data-brand="WELLA">+ В заявку</button></td></tr>
<tr><td>3</td><td>Brilliance Бальзам-уход для жёстких волос, 1000 мл</td><td>339313</td><td class="col-price-cell">2824.55 ₽</td><td class="col-order-cell"><button type="button" class="cart-add-btn" data-name="Brilliance Бальзам-уход д/защиты цвета окр.жестк.волос 1000мл 339313" data-price="2824.55" data-brand="WELLA">+ В заявку</button></td></tr>
<tr><td>4</td><td>Brilliance Шампунь для нормальных и тонких волос, 300 мл</td><td>339221</td><td class="col-price-cell">919.7 ₽</td><td class="col-order-cell"><button type="button" class="cart-add-btn" data-name="Brilliance Шампунь д/защиты цвета окр.норм. и тонких волос 300мл 339221" data-price="919.7" data-brand="WELLA">+ В заявку</button></td></tr>
<tr><td>5</td><td>Brilliance Шампунь для нормальных и тонких волос, 1000 мл</td><td>339283</td><td class="col-price-cell">2047.48 ₽</td><td class="col-order-cell"><button type="button" class="cart-add-btn" data-name="Brilliance Шампунь д/защиты цвета окр.норм. и тонких волос 1000мл 339283" data-price="2047.48" data-brand="WELLA">+ В заявку</button></td></tr>
<tr><td>6</td><td>Brilliance Шампунь для жёстких волос, 300 мл</td><td>339238</td><td class="col-price-cell">919.7 ₽</td><td class="col-order-cell"><button type="button" class="cart-add-btn" data-name="Brilliance Шампунь д/защиты цвета окр.жестк.волос 300мл 339238" data-price="919.7" data-brand="WELLA">+ В заявку</button></td></tr>
<tr><td>7</td><td>Brilliance Шампунь для жёстких волос, 1000 мл</td><td>339290</td><td class="col-price-cell">2047.48 ₽</td><td class="col-order-cell"><button type="button" class="cart-add-btn" data-name="Brilliance Шампунь д/защиты цвета окр.жестк.волос 1000мл 339290" data-price="2047.48" data-brand="WELLA">+ В заявку</button></td></tr>
</tbody>
</table>
</div>

## Палитра оттенков

<div class="callout"><b>Соответствие кодов между брендами.</b> Номерная система (глубина/тон) у Wella и Londa общая для деми-перманентных безаммиачных линеек — один и тот же код на Color Touch и на <a href="/articles/2026-08-29-londa-professional/#demi-line">Londa Demi-Permanent Ammonia Free</a> означает один и тот же результат. При отсутствии нужного оттенка код можно использовать у обоих брендов.</div>

Палитра Color Touch разработана так, чтобы точно совпадать с оттенками Koleston Perfect и служить деми-перманентным дополнением к перманентному окрашиванию: Pure Naturals, Rich Naturals, Deep Browns, Vibrant Reds, Special Mix, Plus.

